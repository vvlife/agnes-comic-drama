# 🎭 Agnes 漫剧生成器

> 基于 [Agnes AI](https://platform.agnes-ai.com) 免费全模态 API，零成本生成 AI 漫剧。

**剧本 → 角色卡 → 分镜关键帧 → 图生视频 → TTS配音 → 成片拼接**，全流程免费。

### 🌐 在线体验

无需安装，直接打开：**👉 [https://agnes-comic-drama.vercel.app](https://agnes-comic-drama.vercel.app)**

> 已内置 API KEY，打开即可一键生成你的 AI 漫剧。

---

## 🎬 效果演示

下面是 6 个示例主题生成的成片预览（GIF 动图，在 GitHub 内可直接看到循环播放；点链接可看完整视频）：

### 1. 少年剑仙三年归来
![少年剑仙三年归来](docs/gifs/ex_sword-return.gif)
[▶ 观看完整视频](https://agnes-comic-drama.vercel.app/api/static-video/ex_sword-return.mp4)

### 2. 深宫妃子博弈
![深宫妃子博弈](docs/gifs/ex_palace.gif)
[▶ 观看完整视频](https://agnes-comic-drama.vercel.app/api/static-video/ex_palace.mp4)

### 3. 暴雨夜客栈
![暴雨夜客栈](docs/gifs/ex_rainy-inn.gif)
[▶ 观看完整视频](https://agnes-comic-drama.vercel.app/api/static-video/ex_rainy-inn.mp4)

### 4. 社畜周报封印魔尊
![社畜周报封印魔尊](docs/gifs/ex_excel-shen.gif)
[▶ 观看完整视频](https://agnes-comic-drama.vercel.app/api/static-video/ex_excel-shen.mp4)

### 5. 少女穿越画中世界
![少女穿越画中世界](docs/gifs/ex_painting-world.gif)
[▶ 观看完整视频](https://agnes-comic-drama.vercel.app/api/static-video/ex_painting-world.mp4)

### 6. 最后武林大会
![最后武林大会](docs/gifs/ex_martial-meet.gif)
[▶ 观看完整视频](https://agnes-comic-drama.vercel.app/api/static-video/ex_martial-meet.mp4)

> 想生成你自己的漫剧？按下方「快速开始」配置 API KEY 即可一键出片。

### 🖥️ 平台界面

![漫剧平台界面](docs/screenshots/platform-ui.png)

---

## ✨ 特性

- 🆓 **完全免费** — 使用 Agnes AI 免费额度（RPM ≤ 20），不花一分钱
- 🎬 **全流程** — 一键从主题到成片，自动执行所有步骤
- 🎨 **多风格** — 三渲二国风、水墨、赛博朋克、日系动漫
- 🎭 **多类型** — 仙侠、宫斗、江湖、都市
- 📊 **实时进度** — Web UI 可视化进度条 + 实时日志
- 🔧 **续跑支持** — Checkpoint 机制，失败后不重做已完成步骤
- 🎙️ **TTS 配音** — edge-tts 跨平台语音合成，自动角色分配
- 📧 **邮件通知** — 生成完成后自动发送邮件（可选）
- 🔑 **本地配置** — API KEY 仅保存在本地，不上传任何服务器
- 🎞️ **多轨剪辑器** — 时间轴拖拽排序、逐镜裁剪、音轨静音/独奏、字幕显隐、AI 剪辑助手、实时预览
- ⏳ **加载缓冲** — 剪辑器打开时显示加载动画，避免长加载白屏
- 🪟 **导出弹窗** — 点击「导出」弹出执行中弹窗，实时刷新进度与日志，完成后跳转成片页

---

## 🚀 快速开始

### 1. 安装依赖

```bash
pip install flask flask-cors edge-tts
```

> 视频拼接需要本地安装 [FFmpeg](https://ffmpeg.org/)。

### 2. 获取 API KEY

前往 [platform.agnes-ai.com](https://platform.agnes-ai.com) 免费注册，获取 API KEY。

### 3. 启动 Web 服务

```bash
cd agnes-comic-drama/web
python3 server.py --port 7788
```

打开浏览器访问 **http://127.0.0.1:7788**，点击右上角 ⚙️ 配置 API KEY，即可开始生成。

### 3'. 命令行方式（可选）

```bash
export AGNES_API_KEY="sk-your-key-here"
cd agnes-comic-drama/scripts
python3 run.py --theme "少年剑仙三年归来" --style "三渲二国风" --genre "仙侠" --duration 15
```

---

## 📁 项目结构

```
agnes-comic-drama/
├── SKILL.md                  # 技能定义
├── README.md                 # 本文件
├── .gitignore
├── requirements.txt          # Python 依赖
├── scripts/
│   ├── agnes_client.py       # Agnes API 封装（文本/图像/视频）
│   ├── run.py                # 命令行全流程入口
│   └── supabase_storage.py   # Supabase 持久化（Vercel 用）
├── api/
│   ├── index.py              # Vercel Serverless 版本
│   └── index.html            # Vercel 前端
├── web/
│   ├── server.py             # Flask Web 后端
│   ├── static/               # 旧版前端
│   └── config.json           # 本地配置（API KEY，git 已忽略）
├── public/
│   ├── index.html            # 新版前端 SPA（生成器）
│   ├── editor.html           # 多轨剪辑器页面
│   └── result.html           # 成片展示页（下载 / 复制预览链接）
└── vercel.json               # Vercel 路由配置
```

---

## 🔧 配置

API KEY 优先级（从高到低）：

1. **Web UI 配置页面**（保存到 `web/config.json`）
2. **环境变量** `AGNES_API_KEY`
3. **OpenClaw 配置** `~/.qclaw/openclaw.json`

邮件配置（可选）：
- `RESEND_API_KEY` — Resend API Key
- `EMAIL_FROM` — 发件人地址

所有配置仅保存在本地，不会上传到任何远程服务器。

---

## 🎬 生成流程

| Step | 内容 | API 模型 | 免费 |
|------|------|----------|------|
| 1 | 📝 剧本生成 | agnes-2.0-flash | ✅ |
| 2 | 🎨 角色三联卡 | agnes-image-2.1-flash | ✅ |
| 3 | 🖼️ 分镜关键帧 | agnes-image-2.1-flash | ✅ |
| 4 | 📹 图生视频 | agnes-video-v2.0 | ✅ |
| 5 | 🎙️ TTS 配音 | edge-tts（本地） | ✅ |
| 6 | 🔊 音效描述 | agnes-2.0-flash | ✅ |
| 7 | 🎚️ 音频混音 | FFmpeg（本地） | ✅ |
| 8 | 🎬 成片拼接 | FFmpeg（本地） | ✅ |

---

## 🎞️ 多轨剪辑器

生成完成后，可从生成器打开 **多轨剪辑器**（`/editor.html?project=<项目ID>`）对成片进行精细化剪辑。

### 功能

- **时间轴编辑** — V1 视频轨拖拽排序，逐镜裁剪入点/出点（拖拽片段边缘或填数值）
- **多音轨** — 配音（🎙）/ 场景音（🌊）/ 音乐（🎵）各自独立裁剪、静音、独奏、调音量，可上传自定义音乐
- **字幕轨** — 与视频逐镜对齐，支持单段显隐与全局 CC 开关
- **实时预览** — 预览监视器按当前顺序/裁剪/静音实时连放整片，时间轴红色播放头同步移动
- **AI 剪辑助手** — 右下角 🤖 浮窗，用自然语言指挥剪辑（如「把第 2 段视频缩短到 3 秒」「静音场景音」）

### 页面行为

- **加载缓冲** — 打开剪辑器时显示「加载中」动画（拉取场景/音轨/波形），数据就绪后自动淡出，避免白屏等待
- **导出弹窗** — 点击「导出」后弹出居中弹窗，实时刷新执行状态与日志；完成后自动跳转成片页
- **成片页** — 导出完成后跳转到 `/result.html`，在该页查看成片视频，并提供**下载成片**与**复制预览链接**按钮（剪辑器自身不再展示成片区块）

---

## 🧩 API 模型映射

| 用途 | 模型 | 说明 |
|------|------|------|
| 文本生成 | `agnes-2.0-flash` | 256K 上下文，支持 Thinking 模式 |
| 图像生成 | `agnes-image-2.1-flash` | 文生图 + 图生图（多图参考） |
| 视频生成 | `agnes-video-v2.0` | 图生视频，异步任务 |

Base URL: `https://apihub.agnes-ai.com/v1`

---

## 🌐 部署

### 本地部署

```bash
cd web
python3 server.py --host 0.0.0.0 --port 8080
```

### Vercel 部署

1. Fork 本仓库
2. 在 Vercel Dashboard 添加环境变量 `AGNES_API_KEY`
3. 自动部署

### 服务器部署

```bash
# 安装依赖
apt-get install -y ffmpeg fonts-wqy-zenhei fonts-wqy-microhei
pip install flask flask-cors edge-tts

# 启动服务
nohup python3 web/server.py --host 0.0.0.0 --port 8080 > /tmp/agnes-comic-drama.log 2>&1 &
```

---

## ⚠️ 注意事项

- **RPM 限制**：免费账户 20 次/分钟，生成过程自动限流
- **视频生成**：异步任务，需轮询查询结果（使用 `video_id`）
- **图生图**：`image` 参数必须放在 `extra_body` 中
- **视频时长**：`num_frames` ≤ 441 且满足 8n+1（81/121/161/241/441）
- **角色一致性**：通过 Agnes Image 多图参考（最多 4 张角色卡）
- **中文字幕**：服务器需安装中文字体（`fonts-wqy-zenhei`）
- **邮件功能**：需配置 `RESEND_API_KEY`，视频附件限制 50MB

---

## 📜 License

MIT

---

## 🙏 致谢

- [Agnes AI](https://platform.agnes-ai.com) — 免费全模态 API
- [FFmpeg](https://ffmpeg.org/) — 视频处理
- [edge-tts](https://github.com/rany2/edge-tts) — 语音合成
